...
/*public class MyJDBCStateRepo extends MyJDBCStateRepo {

	public MyJDBCStateRepo(DataSource dataSource) {
		super(dataSource);
 ...
 * 
 * <p>
 * {@link MyJDBCStateRepo} stores the feature state in a single database table. You can choose the name of this table using
 ...
 * 
 * <pre>
 * StateRepository repository = MyJDBCStateRepo.newBuilder(dataSource)
 *     .tableName(&quot;features&quot;)
 *     .createTable(false)
...
public class MyJDBCStateRepo implements StateRepository {
    ...
    protected final Log log = LogFactory.getLog(MyJDBCStateRepo.class);
    ...
    /**
     * Constructor of {@link MyJDBCStateRepo}. A database table called <code>TOGGLZ</code> will be created automatically for
     * you.
     * 
     * @param dataSource The JDBC {@link DataSource} to obtain connections from
     * @see #MyJDBCStateRepo(DataSource, String)
     */
    public MyJDBCStateRepo(DataSource dataSource) {
    ...
    /**
     * Constructor of {@link MyJDBCStateRepo}. The database table will be created automatically for you.
     * 
     * @param dataSource The JDBC {@link DataSource} to obtain connections from
     * @param tableName The name of the database table to use
     */
    public MyJDBCStateRepo(DataSource dataSource, String tableName) {
        this(new Builder(dataSource).tableName(tableName));
    }

    /**
     * Constructor of {@link MyJDBCStateRepo}.
     * 
     * @param dataSource The JDBC {@link DataSource} to obtain connections from
     * @param tableName The name of the database table to use
     * @param createTable If set to <code>true</code>, the table will be automatically created if it is missing
     * @deprecated use {@link MyJDBCStateRepo#newBuilder(DataSource)} to create a builder that can be used to configure all
     *             aspects of the repository in a fluent way
     */
    @Deprecated
    public MyJDBCStateRepo(DataSource dataSource, String tableName, boolean createTable) {
     ...
     */
    @Deprecated
    public MyJDBCStateRepo(DataSource dataSource, String tableName, boolean createTable, MapSerializer serializer) {
     ...
     */
    private MyJDBCStateRepo(Builder builder) {
    ...
    protected void migrateSchema() {
        ...
        try {
            ...
            try {
                ...
                MySchemaUpdater updater = new MySchemaUpdater(connection, tableName, serializer);
                ...
                try {
                    ...
                    try {
                        ...
                        if (resultSet.next()) {
                            ...
                            boolean enabled = resultSet.getInt(MyColumns.FEATURE_ENABLED) > 0;
                            ...
                            String paramData = resultSet.getString(MyColumns.STRATEGY_PARAMS);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    updateStatement.setString(4, featureState.getFeature().name()); //4
                ...
                if (updatedRows == 0) {
                    ...
                    String insertSql = "INSERT INTO %TABLE% (FEATURE_NAME, FEATURE_ENABLED, STRATEGY_ID, STRATEGY_PARAMS) (?,?,?,?)";
    ...
    /**
     * Creates a new builder for creating a {@link MyJDBCStateRepo}.
     * 
    ...
    /**
     * Builder for a {@link MyJDBCStateRepo}.
     */
    public static class Builder {
        ...
        /**
         * Creates a {@link MyJDBCStateRepo} from the current configuration
         */
        public MyJDBCStateRepo build() {
            return new MyJDBCStateRepo(this);
        }
